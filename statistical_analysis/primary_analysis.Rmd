...
# crude
fit_crude_rcs_imp <- lrm(vycurrentasthma_dev ~ rcs(rsv_pcr_age, 3), data=df_sing_imp)
...
df_cat_imp <- df_cat_imp %>% 
  mutate(rsv_age_cat = case_when(rsv_pcr_age < 100 | 200 < rsv_pcr_age ~ 0,
                                 100 <= rsv_pcr_age & rsv_pcr_age <= 200 ~ 1)) %>% 
  pivot_longer(cols = c(oy_rec_whz_dev, ty_rec_whz_dev, fy_rec_whz_dev, vy_rec_whz_dev, xy_rec_whz_dev),
               names_to = "recurrent_wheeze_type",
               values_to = "recurrent_wheeze")

fit_gee_imp <- geeglm(recurrent_wheeze ~ factor(rsv_age_cat),
                      id = cfsubjid,
                      std.err = "san.se",
                      corstr = "exchangeable",
                      family = binomial(link = "logit"),
