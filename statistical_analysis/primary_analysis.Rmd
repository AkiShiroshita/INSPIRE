...
gg_miss_var(df_tab %>% dplyr::select(
         ...
         BIRTHWEIGHT_GRAM, DELIVERYMODE, EVERBREASTFED, DAYCARE_OY, other_sib_cat, oy_rsv_infected_dev, rsv_pcr_age,
         vycurrentasthma_dev)) 
```
...
```{r}
df_tab2 <- df_tab %>% 
  filter(!is.na(vycurrentasthma_dev) & !is.na(rsv_pcr_age))
