...
df_sing_imp <- df_sing_imp2 <- df %>% 
  mutate(enroll_age = trunc(time_length(interval(INFANTDOB, STUDYENROLLDATE), "month")),
         ...
         rsv_sero_age = trunc(time_length(interval(INFANTDOB, lmday1), "day"))) %>% 
  filter(oy_rsv_infected_dev == 1) %>% 
  dplyr::select(cfsubjid, fycurrentasthma_dev, vycurrentasthma_dev, rsv_pcr_age, rsv_pcr_age_ori, sex, INFANTDOB, lmday1, oy_rec_whz_dev, ty_rec_whz_dev, ry_rec_whz_dev, fy_rec_whz_dev, vy_rec_whz_dev, xy_rec_whz_dev, oy_rsv_infected_dev, GESTAGEWK, DAYCARE_OY, MATSMOKE_PREG, MAT_ASTHMA_EVER, EVERBREASTFED, MAT_BMI_PREPREG, DELIVERYMODE, OTHERLIVSIBS_BIO) %>% 
  arrange(INFANTDOB)
